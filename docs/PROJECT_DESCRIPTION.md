...
Дата: 14 сентября 2026 года. Статус: этапы 0–2 реализованы в исходном объёме; группы моделей (2A), выполнение процессов и дальнейшие продуктовые сценарии ожидают следующих этапов.

Связанный документ: [план реализации](IMPLEMENTATION_PLAN.md). Редакция 3: добавлены группы моделей с приоритетами и автоматическим переключением для агентов и обычных LLM; сохранены уточнения контрактов и внутреннего MVP из редакции 2.
...
Требования к группам моделей определены в §7.5 этого документа. Они уточняют прежний запрет автоматической замены: переход внутри заранее выбранной группы разрешён, выбор вне её требует нового Run. Синхронизация архитектурных контрактов с этим расширением запланирована в этапе 2A плана реализации; наличие требований не означает, что функция уже реализована.
