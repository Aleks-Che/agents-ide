...
Дата: 14 сентября 2026 года. Статус: каркас этапов 0–1 реализован; продуктовые сценарии ниже ожидают реализации следующих этапов.

Связанный документ: [план реализации](IMPLEMENTATION_PLAN.md). Редакция 2: уточнены контракты исполнения и внутренний MVP по результатам ревью.
...
- [Состояния и команды](docs/architecture/STATE_MACHINES.md): Run, посещения, попытки, гонки, идемпотентность.
- [Runtime](docs/architecture/RUNTIME_CONTRACTS.md): очередь, SQLite, SSE, события/ошибки, процессы, capability, бюджеты и retry.
- [Исполнение](docs/architecture/EXECUTION_CONTRACTS.md): схема графа, Command/CollectContext, план, проверки и Git.
- [Безопасность и эксплуатация](docs/architecture/SECURITY_AND_OPERATIONS.md): доверие, авторизация, секреты, пути, хранение, backup и поставка.
...
- Выбор Codex/OpenCode и доступной модели для каждого агентного шага.
...
- Запуски разных проектов могут идти одновременно в пределах общего лимита. Для одного рабочего каталога действует очередь и исключительное владение.
...
После первого выпуска: параллельные ветви и их объединение, удалённые workers, несколько пользователей, расписания, автоматические PR/push, запуск в отдельных Git worktree и дополнительные LLM-протоколы. Эти возможности не требуются для завершения первой версии.
...
| StepAttempt | Отдельная попытка исполнения посещения: технический повтор или продолжение после остановки |
...
Сообщение становится заданием только после явного действия «Отправить агенту» или «Запустить pipeline». Одиночный запрос агенту использует тот же движок как pipeline из одного рабочего шага. Сообщения во время активного Run сохраняются заметками чата и не меняют его входы. Новая задача, критерии или модель требуют нового Run; недостающие evidence можно предоставить как resolution-артефакты в рамках существующих разрешений.
...
Запуск получает неизменяемый снимок версии графа, входов/плана, промптов, выбранных моделей и разрешённых действий. Изменение шаблона влияет на будущие запуски. Продолжение сохраняет снимок; увеличение лимитов оформляется отдельной RunPolicyRevision и не сбрасывает использование. Структурное редактирование, смена модели или расширение прав требуют нового запуска.

Перед стартом preflight показывает итоговые настройки с источником, недостающие входы, состояние подключений, Git-план и разрешения. Preflight не заменяет повторную проверку изменяемой среды worker под резервацией.

### 4.3. Наблюдение

Пользователь видит текущий узел и переход, номер цикла и попытки, исполнителя, модель, время начала, длительность, сообщения агента, доступные события инструментов, изменения файлов и результат проверки.
...
| AgentTask | Codex/OpenCode, модель, промпты, роль, разрешения, политика сессии, формат результата |
| LLMRequest | Подключение, модель, system/user prompt, контекст, параметры генерации, формат результата |
...
- `plan`: исходный текст/файл плана и фиксированный список PlanItem с ID и критериями. Пользователь подтверждает разбивку до старта; без разбивки весь текст становится одним пунктом P1. Сервер выбирает следующий незавершённый ID по порядку, агент не пересоздаёт план на каждой итерации.
- `implementer`, `reviewer`, `plan_checker`: профили harness и выбранные модели. Проверку всего плана в исходном пресете выполняет агент.
- `verifier`: агент либо обычная LLM. По умолчанию используется агент, пока пользователь не выбрал LLM-подключение.
...
Пресет готов по структуре, переходам, схемам ответов и промптам. Перед первым запуском пользователь связывает роли с действительно доступными профилями и моделями. Названия моделей и ключи не зашиваются в пресет.
...
Каталог получает бэкенд из конкретного harness/провайдера; TTL=15 минут, смена версии harness или подключения инвалидирует кэш. Сохраняются точный ID, источник, параметры и дата. Каталог не гарантирует доступ: его проверяет диагностика/вызов. Исчезнувшая модель не заменяется автоматически, ручной ID помечается unverified. Полная v1 должна подтвердить автоматические каталоги обоих harness.
...
| pause_requested / paused | Запрошена пауза после текущего шага / достигнута граница и работа приостановлена |
...
«Пауза» даёт текущему шагу завершиться и запрещает запуск следующего. «Остановить» прерывает текущий вызов; `stopped` выставляется после подтверждения исполнителя или завершения дерева процессов. «Продолжить» из paused запускает следующий шаг, из stopped создаёт новую попытку незавершённого шага с сохранённым контекстом. Из waiting_input продолжение доступно после устранения указанной причины. Отмена активного запуска также проходит через подтверждённую остановку.
...
Приоритет настроек: явное переопределение запуска → привязка в проекте → версия pipeline → глобальные значения по умолчанию. Параметры конкретного узла могут переопределять настройки выбранной роли; окончательное разрешение всех значений сохраняется при старте. Ссылки на секреты разрешаются worker при вызове и не копируются в снимок.
