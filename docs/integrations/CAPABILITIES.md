...
## Структурированный pipeline, 19.09.2026

Позднее, по указанию пользователя, тот же pipeline/single_agent прошёл с настоящей
бесплатной `opencode/ling-3.0-flash-fin-free` через штатный Zen provider: три AgentTask,
три разных native sessions, три валидных JSON-ответа, все процессы остановлены.
Обычный LLMRequest остался локальным: free Zen отклоняет прямой API вызов вне OpenCode.
[Результат](fixtures/2026-09-19-zen-runtime.json). Разрешённая запись и полный MVP
этой проверкой не подтверждены; ранний проход ниже был полностью синтетическим.

OpenCode 1.18.31 выполнил production pipeline с двумя AgentTask и LLMRequest,
а также отдельный single_agent Run. Проверены три разные native sessions,
валидный структурированный JSON, переключение агентной группы после недоступной
модели и LLM-группы после HTTP 401. Все процессы завершены. Provider локальный
синтетический, внешних модельных вызовов **0**; это подтверждение транспорта,
не качества/доступности внешних моделей или разрешённой записи.
[Отчёт](fixtures/2026-09-19-native-runtime.json),
[воспроизведение и открытые условия](../operations/ACCEPTANCE_2026_09_19.md).

Повторно прошли native provider auth OpenCode 1.18.31 и private sandbox Codex
0.153.4: чтение контекста разрешено, соседний synthetic `.env` и запись запрещены.
B-001/B-002 и полная межharness-приёмка сохраняются.
