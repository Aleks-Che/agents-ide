...
**Статус:** заявленная часть проверена и исправлена. Этап 9 остаётся частичным:
пункты выбора исполнителя, параметров моделей и direct/group ещё не завершены.
Отметки harness и групп в плане возвращены в `[ ]` с перечислением готовой части.
Переименование проекта реализовано, поэтому пункт форм проектов/чатов закрыт.
Эта запись уточняет первичные отчёты ниже, включая замечания о версиях форм,
ключах и запуске Playwright.
