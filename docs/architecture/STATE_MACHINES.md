...
В режиме `workspace_mode=worktree` snapshot фиксирует исходный коммит и путь будущей папки.
Разные управляемые worktree одного репозитория получают независимые резервации; обычный
checkout сохраняет исключительную блокировку репозитория. Worker сохраняет намерение до
`git worktree add`, затем идентичность созданного каталога в runtime. Resume и recovery
используют эту идентичность, не меняя исходный snapshot. Worktree сохраняется после
терминального состояния; незавершённый или изменённый checkout автоматически не перезаписывается.
...
PlanningJob отделён от Run: drafting → merging → needs_answers | ready_for_confirmation → confirmed. Ответы/ручная правка создают новую ревизию в needs_answers или ready_for_confirmation. Отмена из любого нетерминального состояния даёт cancelled; отсутствие кворума, лимит, отказ merger или неизвестный внешний исход — failed с причиной. Confirmed/cancelled терминальны. Failed → drafting допускается только явным retry с expected_state_version, оставшимся общим бюджетом и завершённым/истёкшим lease; generation отсекает старый worker. Unknown требует отдельного согласия на возможное повторное исполнение. Бюджет не превращается в отмену пользователя и не сбрасывается при retry. Cancel сверяет state_version, ответы/confirm — номер ревизии; это разные счётчики. Полный контракт retry и остающиеся harness-сценарии: [PLANNING_COUNCIL](PLANNING_COUNCIL.md).

При council_quorum_missing допускается отдельный failed → needs_answers | ready_for_confirmation: явный promote_single с confirm_degraded и ровно одним проверенным черновиком. Переход снимает завершённый/истёкший lease, увеличивает generation, очищает текущие ошибку/finished_at и сохраняет прежние диагностические события. Он не делает внешних вызовов и не возобновляет бюджет. Ответы и отдельное confirm по-прежнему обязательны перед Run; degraded сохраняется в последующих ревизиях и snapshot.
