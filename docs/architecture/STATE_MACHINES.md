...
Статус: контракт для реализации v1; проверка кодом ещё не выполнена. Связанные документы: [описание](../../PROJECT_DESCRIPTION.md), [runtime](RUNTIME_CONTRACTS.md).
...
- В Run одновременно не более одной незавершённой внешней попытки; запрос разрешения harness также считается незавершённой попыткой.
...
Исходные input, план, модели, промпты, граф и разрешения Run неизменяемы. Новые сообщения чата не меняют их. Недостающие данные передаются как отдельные resolution-артефакты в рамках существующих критериев и разрешённых источников. Изменение задачи, критериев, модели или разрешений требует нового Run.
