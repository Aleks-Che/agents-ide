...
export function GroupSummarySection({
  ...
  return (
    <section className="run-group-summary" aria-label="Группа и кандидаты">
      ...
      <p className="hint">
        Используются общие группы из настроек. Изменения состава, порядка и
        параметров применяются перед следующим обращением к модели; текущий
        запрос продолжает работу.
      </p>
      {groups.length ? (
      ...
      {exhausted ? (
        <div className="candidate-diagnostics">
          ...
          <p>
            Причина для каждой позиции указана в таблице узла. После изменения
            общей группы или восстановления доступа нажмите «Продолжить»:
            начнётся выбор из актуального состава группы.
          </p>
        </div>
      ) : null}
      {nodes.map((node) => (
