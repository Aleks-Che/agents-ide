...
Локальная рабочая область для проектов, чатов и процессов с агентами. Реализованы каркас **этапа 1** и конфигурационные API **этапа 2**: FastAPI, независимый worker, SQLite, локальный вход, DPAPI и React-интерфейс состояния служб. Результаты реальных проверок **этапа 0** и оставшиеся `unverified` перечислены в [матрице интеграций](docs/integrations/CAPABILITIES.md).
