...
Этап 3 добавляет [схемы, AST и серверную валидацию графов](docs/architecture/GRAPH_VALIDATION.md), ограниченные циклы, импорт/экспорт, доверие итоговому hash и конфигурационный preflight. Реальные capability, права и повторная проверка перед dispatch остаются этапам 4–8; успешный preflight пока не означает готовность worker к выполнению.
